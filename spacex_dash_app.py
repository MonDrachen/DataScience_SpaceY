...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        filtered_df = spacex_df[spacex_df['class'] == 1].groupby('Launch Site')['class'].sum().reset_index()
        ...
        names={0: 'Failed', 1:'Success'},
        color='class',
        color_discrete_map={
                0: "red",
                1: "gblue"},
        title="Launche's Outcomes in {}".format(entered_site))
...
def get_scatter_plot(entered_site, payload_range):
    ...
    if entered_site == 'ALL':
        fig = px.scatter(spacex_df[mask], x='Payload Mass (kg)', y='class', color = 'Booster Version Category',
                        title='Correlation between Payload and Successful Landing')
    else:
        # return the outcomes piechart for a selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.scatter(filtered_df[mask], x='Payload Mass (kg)', y='class', color = 'Booster Version Category',
                        title='Correlation between Payload and Successful Landing in {}'.format(entered_site))
